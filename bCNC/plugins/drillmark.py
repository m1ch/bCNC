...
class Tool(Plugin):
    ...
    def appendBurn(self, app, block):
        ...
        block.append(globCNC.grapid(x=x0, y=y0))
        block.append(f"g1 m3 {globCNC.fmt('s', burnpower)}")
        block.append(f"g4 {globCNC.fmt('p', burntime)}")
        block.append("m5")

    def getPowerLine(self, app):
        if globCNC.laseradaptive:
            pwrcode = "m4"
        else:
            pwrcode = "m3"
        markpower = self["Mark power"]
        return f"{pwrcode} {globCNC.fmt('s', markpower)}"

    def appendMark(self, app, block):
        def appendCross(block):
            block.append("m5")
            block.append(globCNC.grapid(x=x0, y=y0 + marksizehalf, f=movefeed))
            block.append(self.getPowerLine(app))
            block.append(globCNC.gline(y=y0 - marksizehalf, f=drawfeed))
            block.append("m5")
            block.append(globCNC.grapid(x=x0 + marksizehalf, y=y0, f=movefeed))
            block.append(self.getPowerLine(app))
            block.append(globCNC.gline(x=x0 - marksizehalf, f=drawfeed))
            block.append("m5")

        def appendCross45(block):
            msh = marksizehalf * self.sin45
            block.append("m5")
            block.append(globCNC.grapid(x=x0 - msh, y=y0 + msh, f=movefeed))
            block.append(self.getPowerLine(app))
            block.append(globCNC.gline(x=x0 + msh, y=y0 - msh, f=drawfeed))
            block.append("m5")
            block.append(globCNC.grapid(x=x0 + msh, y=y0 + msh, f=movefeed))
            block.append(self.getPowerLine(app))
            block.append(globCNC.gline(x=x0 - msh, y=y0 - msh, f=drawfeed))
        ...
        def appendCircle(block):
            block.append("m5")
            block.append(globCNC.grapid(x=x0, y=y0 + marksizehalf / 2, f=movefeed))
            block.append(self.getPowerLine(app))
            block.append(
                globCNC.garc(
                    2,
        ...
        def appendSpikes(block):
            ...
            block.append("m5")
            block.append(globCNC.grapid(x=x0, y=y0, f=movefeed))
            block.append(self.getPowerLine(app))
            block.append(
                globCNC.gline(
            ...
            block.append(globCNC.gline(x=x0, y=y0, f=drawfeed))
            block.append(
                globCNC.gline(
        ...
        def appendSpikes45(block):
            ...
            block.append("m5")
            block.append(
                globCNC.grapid(x=x0 + marksizehalf / 2,
                           y=y0 + marksizehalf / 2, f=movefeed)
            )
            block.append(self.getPowerLine(app))
            block.append(globCNC.gline(x=x0 - marksizehalf / 2, f=drawfeed))
            block.append(globCNC.gline(y=y0 - marksizehalf / 2, f=drawfeed))
            block.append(globCNC.gline(x=x0 + marksizehalf / 2, f=drawfeed))
            block.append(globCNC.gline(y=y0 + marksizehalf / 2, f=drawfeed))
            block.append("m5")
    ...
    def execute(self, app):
        ...
        marktype = self["Mark type"]
        block = Block(name + f" {marktype} diameter {globCNC.fmt('', marksize)}")
        ...
        if active == 0:
            active = 1
        blocks = [block]
        globGCode.insBlocks(active, blocks, _("Manual drill mark"))

...
class SimpleRectangle:
    ...
    def calc(self, xstart, ystart, xend, yend, radius, cw):
        ...
        block.append(globCNC.grapid(x=xmin, y=ymin + r))
        block.append(globCNC.grapid(z=0.0))
        block.append("(entered)")
        if cw:
            block.append(globCNC.gline(x=xmin, y=ymax - r))
            if r > 0:
                block.append(globCNC.garc(2, x=xmin + r, y=ymax, i=r, j=0))
            if (xmax - xmin) > 2 * r:
                block.append(globCNC.gline(x=xmax - r, y=ymax))
            if r > 0:
                block.append(globCNC.garc(2, x=xmax, y=ymax - r, i=0, j=-r))
            if (ymax - ymin) > 2 * r:
                block.append(globCNC.gline(x=xmax, y=ymin + r))
            if r > 0:
                block.append(globCNC.garc(2, x=xmax - r, y=ymin, i=-r, j=0))
            if (xmax - xmin) > 2 * r:
                block.append(globCNC.gline(x=xmin + r, y=ymin))
            if r > 0:
                block.append(globCNC.garc(2, x=xmin, y=ymin + r, i=0, j=r))
        else:
            if r > 0:
                block.append(globCNC.garc(3, x=xmin + r, y=ymin, i=r, j=0))
            if (xmax - xmin) > 2 * r:
                block.append(globCNC.gline(x=xmax - r, y=ymin))
            if r > 0:
                block.append(globCNC.garc(3, x=xmax, y=ymin + r, i=0, j=r))
            if (ymax - ymin) > 2 * r:
                block.append(globCNC.gline(x=xmax, y=ymax - r))
            if r > 0:
                block.append(globCNC.garc(3, x=xmax - r, y=ymax, i=-r, j=0))
            if (xmax - xmin) > 2 * r:
                block.append(globCNC.gline(x=xmin + r, y=ymax))
            if r > 0:
                block.append(globCNC.garc(3, x=xmin, y=ymax - r, i=0, j=-r))
            if (ymax - ymin) > 2 * r:
                block.append(globCNC.gline(x=xmin, y=ymin + r))
...
class Tool(Plugin):
    ...
    def execute(self, app):
        ...
        if active == 0:
            active = 1
        globGCode.insBlocks(active, blocks, _("Create Simple Rectangle"))
        app.refresh()

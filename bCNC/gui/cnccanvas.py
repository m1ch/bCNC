...
class cnccanvas(tk.Canvas):
    ...
    def actionWPOS(self, x, y):
        u, v, w = self.image2Machine(x, y)
        globSender.mcontrol._wcsSet(u, v, w)
        self.setAction(ACTION_SELECT)
    ...
    def actionAddOrient(self, x, y):
        ...
        if u is None or v is None:
            ...
            return
        self._orientSelected = len(globGCode.orient)
        globGCode.orient.add(globCNC.vars["wx"], globCNC.vars["wy"], u, v)
    ...
    def gantry(self, wx, wy, wz, mx, my, mz):
        ...
        if (
            ...
            if not self.draw_workarea:
                return
            xmin = self._dx - globCNC.travel_x
            ymin = self._dy - globCNC.travel_y
            xmax = self._dx
            ymax = self._dy
    ...
    def selectMarker(self, item):
        # find marker
        for i, paths in enumerate(globGCode.orient.paths):
            if item in paths:
    ...
    def orientChange(self, marker):
        ...
        if marker >= 0:
            ...
            try:
                for i in globGCode.orient.paths[self._orientSelected]:
    ...
    def showInfo(self, blocks):
        ...
        for bid in blocks:
            ...
            closed, direction = globGCode.info(bid)
    ...
    def initPosition(self):
        ...
        self.delete("all")
        self._cameraImage = None
        gr = max(3, int(globCNC.vars["diameter"] / 2.0 * self.zoom))
        if self.view == VIEW_XY:
    ...
    def drawAxes(self):
        ...
        dx = globCNC.vars["axmax"] - globCNC.vars["axmin"]
        dy = globCNC.vars["aymax"] - globCNC.vars["aymin"]
        d = min(dx, dy)
        try:
    ...
    def drawMargin(self):
        ...
        if globCNC.isMarginValid():
            xyz = [
                (globCNC.vars["xmin"], globCNC.vars["ymin"], 0.0),
                (globCNC.vars["xmax"], globCNC.vars["ymin"], 0.0),
                (globCNC.vars["xmax"], globCNC.vars["ymax"], 0.0),
                (globCNC.vars["xmin"], globCNC.vars["ymax"], 0.0),
                (globCNC.vars["xmin"], globCNC.vars["ymin"], 0.0),
            ]
        ...
        if not globCNC.isAllMarginValid():
            return
        xyz = [
            (globCNC.vars["axmin"], globCNC.vars["aymin"], 0.0),
            (globCNC.vars["axmax"], globCNC.vars["aymin"], 0.0),
            (globCNC.vars["axmax"], globCNC.vars["aymax"], 0.0),
            (globCNC.vars["axmin"], globCNC.vars["aymax"], 0.0),
            (globCNC.vars["axmin"], globCNC.vars["aymin"], 0.0),
        ]
    ...
    def drawGrid(self):
        ...
        if self.view in (VIEW_XY, VIEW_ISO1, VIEW_ISO2, VIEW_ISO3):
            xmin = (globCNC.vars["axmin"] // 10) * 10
            xmax = (globCNC.vars["axmax"] // 10 + 1) * 10
    ...
    def drawOrient(self, event=None):
        ...
        globGCode.orient.clearPaths()
        for i, (xm, ym, x, y) in enumerate(globGCode.orient.markers):
            paths = []
            ...
            globGCode.orient.addPath(paths)

        if self._orientSelected is not None:
            try:
                for item in globGCode.orient.paths[self._orientSelected]:
    ...
    def drawProbe(self):
        ...
        for i, uv in enumerate(self.plotCoords(probe.points)):
            item = self.create_text(
                uv,
                text=f"{probe.points[i][2]:.{globCNC.digits}f}",
                tag="Probe",
                justify="center",
    ...
    def drawPaths(self):
        ...
        try:
            ...
            for i, block in enumerate(globGCode.blocks):
                ...
                for j, line in enumerate(block):
                    ...
                    if n == 0:
                        ...
                        n = 1000
                    try:
                        cmd = globGCode.evaluate(
                            globCNC.compileLine(line), self.app)
                        if isinstance(cmd, tuple):
                            cmd = None
                        else:
                            cmd = globCNC.breakLine(cmd)
                    ...
                    if cmd is None or not drawG:
                        ...
                        block.addPath(path)
                        if start and globCNC.gcode in (1, 2, 3):
                            # Mark as start the first non-rapid motion
                            block.startPath(globCNC.x, globCNC.y, globCNC.z)
                            start = False
                block.endPath(globCNC.x, globCNC.y, globCNC.z)
        except AlarmException:
    ...
    def drawPath(self, block, cmds):
        ...
        globCNC.motionEnd()
        if xyz:
            globCNC.pathLength(block, xyz)
            if globCNC.gcode in (1, 2, 3):
                block.pathMargins(xyz)
                globCNC.pathMargins(block)
            if block.enable:
                if globCNC.gcode == 0 and self.draw_rapid:
                    xyz[0] = self._last
                self._last = xyz[-1]
            else:
                if globCNC.gcode == 0:
...
class CanvasFrame(ttk.Frame):
    def __init__(self, master, app, *kw, **kwargs):
        ttk.Frame.__init__(self, master, *kw, **kwargs)
        self.app = app
    ...
    def createCanvasToolbar(self, toolbar):
        ...
        utils.ToolTip(b, _("Select tool [S]"))
        self.addWidget(b)
        b.pack(side="left")
        ...
        utils.ToolTip(b, _("Pan viewport [X]"))
        b.pack(side="left")
        ...
        utils.ToolTip(b, _("Toggle display of axes"))
        b.pack(side="left")
        ...
        utils.ToolTip(b, _("Toggle display of grid lines"))
        b.pack(side="left")
        ...
        utils.ToolTip(b, _("Toggle display of margins"))
        b.pack(side="left")
        ...
        utils.ToolTip(b, _("Toggle display of probe"))
        b.pack(side="left")
        ...
        utils.ToolTip(b, _("Toggle display of paths (G1,G2,G3)"))
        b.pack(side="left")
        ...
        utils.ToolTip(b, _("Toggle display of rapid motion (G0)"))
        b.pack(side="left")
        ...
        utils.ToolTip(b, _("Toggle display of workarea"))
        b.pack(side="left")
        ...
        utils.ToolTip(b, _("Toggle display of camera"))
        b.pack(side="left")
        ...
        # -----------
        self.drawTime = ttk.Combobox(toolbar, width=3)
        self.drawTime['values'] = [
            "inf", "1", "2", "3", "5", "10", "20", "30", "60", "120"]
        self.drawTime['state'] = 'readonly'
        self.drawTime.bind('<<ComboboxSelected>>', self.drawTimeChange)
        # self.drawTime = tkextra.Combobox(
        #     toolbar, width=3, background="White", command=self.drawTimeChange
        # )
        # self.drawTime.fill(
        #     ["inf", "1", "2", "3", "5", "10", "20", "30", "60", "120"])
        utils.ToolTip(self.drawTime, _("Draw timeout in seconds"))
        self.drawTime.set(DRAW_TIME)
        self.drawTime.pack(side="right")
        ttk.Label(toolbar, text=_("Timeout:")).pack(side="right")

...
class CNCListbox(tk.Listbox):
    def __init__(self, master, app, *kw, **kwargs):
        ...
        # FIXME: self.gcode = app.sender.gcode
    ...
    def fill(self, event=None):
        ...
        del self._items[:]
        y = 0
        for bi, block in enumerate(globGCode.blocks):
    ...
    def copy(self, event=None):
        ...
        for block, line in self.getCleanSelection():
            if line is None:
                jsonobj.append(globGCode.blocks[block].dump())
            else:
                jsonobj.append(globGCode.blocks[block][line])
        ...
        def addLines(lines):
            for line in lines.splitlines():
                ...
                if self._lid is None:
                    self._bid += 1
                    if self._bid > len(globGCode.blocks):
                        self._bid = len(globGCode.blocks)
                    self._lid = MAXINT
                    block = Block()
                    undoinfo.append(globGCode.addBlockUndo(self._bid, block))
                    selitems.append((self._bid, None))
                else:
                    block = globGCode.blocks[self._bid]
                ...
                undoinfo.append(
                    globGCode.insLineUndo(self._bid, self._lid, line))
    ...
    def clone(self, event=None):
        ...
        for i in reversed(sel):
            ...
            if lid is None:
                ...
                undoinfo.append(globGCode.cloneLineUndo(bid, lid))
        globGCode.addUndo(undoinfo)
    ...
    def deleteBlock(self, event=None):
        ...
        for i in reversed(sel):
            ...
            if isinstance(lid, int):
                undoinfo.append(globGCode.delLineUndo(bid, lid))
            else:
                undoinfo.append(globGCode.delBlockUndo(bid))
        globGCode.addUndo(undoinfo)
    ...
    def edit(self, event=None):
        ...
        if edit.value is None or edit.value == txt:
            if lid is None:
                ...
                if not globGCode[bid].enable:
                    self.itemconfig(active, foreground=DISABLE_COLOR)
            return

        if isinstance(lid, int):
            globGCode.addUndo(globGCode.setLineUndo(bid, lid, edit.value))
            self.set(active, edit.value)
            if edit.value and edit.value[0] in ("(", "%"):
                self.itemconfig(active, foreground=COMMENT_COLOR)

        else:
            globGCode.addUndo(globGCode.setBlockNameUndo(bid, edit.value))
            self.set(active, globGCode[bid].header())
            self.itemconfig(active, background=BLOCK_COLOR)
            if not globGCode[bid].enable:
                self.itemconfig(active, foreground=DISABLE_COLOR)
    ...
    def insertBlock(self, event=None):
        ...
        globGCode.addUndo(globGCode.addBlockUndo(bid, block))
        self.selection_clear(0, "end")
        self.fill()
    ...
    def insertLine(self, event=None):
        ...
        if lid is None:
            ...
            lid += 1
        globGCode.addUndo(globGCode.insLineUndo(bid, lid, edit.value))
    ...
    def toggleExpand(self, event=None):
        ...
        for i in reversed(items):
            ...
            blocks.append(bid)
            if expand is None:
                expand = not globGCode[bid].expand
            undoinfo.append(globGCode.setBlockExpandUndo(bid, expand))
    ...
    def _toggleEnable(self, enable=None):
        ...
        for i in items:
            ...
            undoinfo.append(globGCode.setBlockEnableUndo(bid, enable))
        ...
        if undoinfo:
            globGCode.calculateEnableMargins()
            globGCode.addUndo(undoinfo)
            self.activate(active)
            self.yview_moveto(ypos)
            self.winfo_toplevel().event_generate("<<ListboxSelect>>")
    ...
    def commentRow(self, event=None):
        ...
        for i in sel_items:
            ...
            if my_item[1] is not None:
                change = True
                # check for ()
                line = globGCode[my_item[0]][my_item[1]]
                m = mreg.search(line)
                if m is None:
                    globGCode[my_item[0]][my_item[1]] = "(" + line + ")"
                else:
                    globGCode[my_item[0]][my_item[1]] = m.group(1)
        if change:
            self.fill()
    ...
    def joinBlocks(self, event=None):
        ...
        sel_items = list(map(int, self.curselection()))
        change = True
        bl = Block(globGCode[sel_items[0]].name())
        for bid in sel_items:
            for line in globGCode[bid]:
                bl.append(line)
            bl.append("( ---------- cut-here ---------- )")
        del bl[-1]
        globGCode.addUndo(globGCode.addBlockUndo(bid + 1, bl))
        if change:
            self.fill()
        self.deleteBlock()
        self.winfo_toplevel().event_generate("<<Modified>>")
    ...
    def changeColor(self, event=None):
        ...
        try:
            rgb, color = tkextra.askcolor(
                title=_("Color"),
                initialcolor=globGCode[bid].color,
                parent=self
            )
        except tk.TclError:
            color = None
        ...
        for i in reversed(items):
            ...
            if lid is not None:
                if bid in blocks:
                    continue
            blocks.append(bid)
            oldColor = globGCode[bid].color
            undoinfo.append(globGCode.setBlockColorUndo(bid, oldColor))

        if undoinfo:
            globGCode.addUndo(undoinfo)
            for bid in blocks:
                globGCode[bid].color = color
            self.winfo_toplevel().event_generate("<<Modified>>")
        self.winfo_toplevel().event_generate(
        ...
        for bi in items:
            ...
            if double:
                if block.expand:
                    ...
                    name = block.nameNop()
                    for i, bl in enumerate(globGCode.blocks):
                        if name == bl.nameNop():
    ...
    def invertBlocks(self, event=None):
        ...
        if not blocks:
            return
        globGCode.addUndo(globGCode.invertBlocksUndo(blocks))
        self.fill()
